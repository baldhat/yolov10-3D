...
class Run:
    ...
    @staticmethod
    def get_flops_(model, imgsz=[1280, 384], batch_sizes=[1,1,1,1,1]):
        import torch
        ...
        with torch.inference_mode():
            ...
            for bs in batch_sizes:
                ...
                print(f"Batch size: {bs} Took: {(t2-t1) / 100 * 1000:.2f}ms, FLOPs: {flops:.2f} GFLOPs, batch size: {im.shape[0]}, peak: {peak}, reserved: {reserved}")
                # for x in range(1000):
                #     # model(im)
                #     model.model[-1](fs)
                # t1 = time.time()
                ...
                #print(f"Head: {bs} Took: {(t2-t1) / 100 * 1000:.2f}ms, FLOPs: {flops:.2f} GFLOPs, memory peak: {peak}, memory reserved: {reserved}")
