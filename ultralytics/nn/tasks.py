...
from ultralytics.utils.loss import v8ClassificationLoss, v8DetectionLoss, v8OBBLoss, v8PoseLoss, v8SegmentationLoss, v10DetectLoss, DetectLoss3d
...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None, return_features=[20]):
        """
        Perform a forward pass through the network.
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        y, dt, embeddings, features = [], [], [], None  # outputs
        for m in self.model:
            ...
            if return_features and m.i in return_features:
                features = x
        if return_features and not isinstance(x, tuple):
            x["features"] = features
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, v10Detect3d)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, v10Detect3d}:
            ...
            if m is v10Detect3d:
                ...
                args.append(d.get("detach_predecessors"))
                args.append(d.get("deform"))
                args.append(d.get("common_head"))
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        if m == "detect" or m == "v10detect" or m == "v10detect3d":
            return "detect"
        if m == "segment":
            return "segment"
        if m == "pose":
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect, v10Detect3d)):
                return "detect"
