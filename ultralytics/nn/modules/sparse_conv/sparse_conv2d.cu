...
__global__ void sparse_conv2d_fast_kernel(
    ...
    const int64_t B, const int64_t C_in,
    const int64_t H_in, const int64_t W_in,
    const int64_t C_out, const int64_t K_h, const int64_t K_w,
    const int64_t stride_h, const int64_t stride_w,
    const int64_t pad_h, const int64_t pad_w,
    const int64_t dilation_h, const int64_t dilation_w,
    const int64_t H_out, const int64_t W_out,
    const int64_t N,
    ...
    int64_t b = idx / (N * C_out);
    int64_t n = (idx / C_out) % N;
    int64_t co = idx % C_out;

    int64_t g = co / (C_out / groups);                // group of this output channel
    int64_t Cin_per_grp = C_in / groups;
    int64_t Cout_per_grp = C_out / groups;

    int64_t ci_start = g * Cin_per_grp;
    int64_t ci_end   = ci_start + Cin_per_grp;
    ...
    int64_t in_y_origin = out_y * stride_h - pad_h;
    int64_t in_x_origin = out_x * stride_w - pad_w;

    // Accumulator
    scalar_t acc = bias ? bias[co] : static_cast<scalar_t>(0);

    // Loop over input channels in this group
    for (int64_t ci = ci_start; ci < ci_end; ++ci) {
        ...
        for (int64_t kh = 0; kh < K_h; ++kh) {
            int64_t in_y = in_y_origin + kh * dilation_h;
            if (in_y < 0 || in_y >= H_in) continue;

            #pragma unroll
            for (int64_t kw = 0; kw < K_w; ++kw) {
                int64_t in_x = in_x_origin + kw * dilation_w;
...
torch::Tensor sparse_conv2d_fast_forward(
    ...
    const int64_t H_out = (H_in + 2 * pad_h - dilation_h * (K_h - 1) - 1) / stride_h + 1;
    const int64_t W_out = (W_in + 2 * pad_w - dilation_w * (K_w - 1) - 1) / stride_w + 1;

    const int64_t N = indices.size(1);

    auto options = input.options();
    auto output = torch::empty({B, C_out, H_out, W_out}, options);

    const int64_t threads = 128;  // smaller threads to increase occupancy
    const int64_t blocks = (B * N * C_out + threads - 1) / threads;
